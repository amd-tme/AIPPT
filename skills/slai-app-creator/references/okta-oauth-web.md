...
2. **Redirect URIs** use your app's public host: **`https://<app_id>.slai-app.amd.com/...`**. Set **`SLAI_PUBLIC_ORIGIN`**, **`SLAI_OAUTH_REDIRECT_URI`**, and related env in your committed **`deployment.yaml`** (or app defaults) — **Platform deploy** applies manifests **as written**; it does **not** patch env at apply time. Align Okta **Sign-in redirect** with the URI the app will use. **Skill / agents:** configure the app to read **`SLAI_OAUTH_REDIRECT_URI`** at runtime (no hardcoded cross-environment redirects in the wrong environment). See **[`manifest-by-namespace.md`](manifest-by-namespace.md)** and [specs/decisions](../../../specs/decisions.md) § *Manifest authority*; you still register **each** app's concrete URIs in Okta. More repo context: [`deploy/README.md`](../../../deploy/README.md).
...
| **Internal AMD XAA detail** | Program-specific POC docs and registration checklists from **security / Eng IT** (not stored in **`slai-app-platform`**). Public background: Okta blog + RFC 8693 in this table. |
